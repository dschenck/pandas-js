...
export class Rolling extends BaseGrouper {
    max() {
        ...
        for (let i = 0; i < this.underlying.length; i++) {
            ...
            if (utils.isNaN(this.underlying._values[i])) {
                values.push(values[values.length - 1])
            }
            else if (utils.isNaN(values[values.length - 1])) {
                values.push(this.underlying._values[i])
            }
            else if (this.underlying._values[i] > values[values.length - 1]) {
                values.push(this.underlying._values[i])
            }
            else if (this.underlying._values[i - this.options.window] < values[values.length - 1]) {
                values.push(values[values.length - 1])
            }
            else {
                values.push(stats.max(this.underlying._values.slice(i - this.options.window + 1, i + 1)))
            }
        }
